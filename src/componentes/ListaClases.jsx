      ...
      <div className="max-w-4xl mx-auto bg-white bg-opacity-90 rounded-3xl shadow-2xl p-8 border border-indigo-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          {!cargando && clasesAMostrar.map(clase => {
            ...
            return (
              <div
                key={clase._id}
                className={`relative block bg-gradient-to-br from-indigo-50 via-pink-50 to-green-50 border rounded-2xl shadow hover:shadow-xl hover:border-indigo-400 transition p-6 ${ninosSinPaquete > 0 ? 'border-red-300 bg-gradient-to-br from-red-50 via-pink-50 to-orange-50' : 'border-indigo-200'
                  }`}
              >
                <Link
                  to={`/clases/${clase._id}`}
                ...
                >
                  <div className="flex items-center gap-2 mb-2">
                    <h3 className="text-xl font-bold text-indigo-700">{clase.nombre}</h3>
                    {clase.bloqueada && (
        ...
        <div className="flex items-center justify-center gap-2">
          <button
            ...
            disabled={pagina === 1}
            className={`px-4 py-2 rounded-xl font-bold transition ${pagina === 1
              ...
              if (
                ...
                return (
                  <button
                    key={p}
                    onClick={() => setPagina(p)}
                    className={`w-10 h-10 rounded-xl font-bold transition ${pagina === p
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl font-bold transition ${pagina === totalPaginas
              ? 'bg-gray-100 text-gray-400 cursor-not-allowed'
              : 'bg-indigo-100 text-indigo-700 hover:bg-indigo-200 shadow-sm'
              }`}
          >
