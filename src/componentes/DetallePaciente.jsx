...
import { ClipboardDocumentListIcon, ArrowLeftIcon, PencilSquareIcon, CreditCardIcon, ArrowDownTrayIcon, TrashIcon } from "@heroicons/react/24/solid";
  ...
  const eliminarPaquete = async (paqueteId) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        setPaquetes(paquetes.filter(p => p._id !== paqueteId));
        ...
        Swal.fire(
          'Error',
          error.message || 'No se pudo eliminar el paquete. Tal vez ya esté siendo usando en alguna clase.',
          'error'
        );
      }
    }
            ...
            {paquetes.length === 0 ? (
              ...
              <div>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                  {paquetes.slice((paginaPaquetes - 1) * itemsPorPagina, paginaPaquetes * itemsPorPagina).map(p => (
                    <div key={p._id} className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm hover:shadow-md transition">
                      <div className="flex justify-between items-start mb-2">
                        <span className="text-sm font-bold text-indigo-700">Factura: {p.numeroFactura}</span>
                        <span className={`text-xs font-bold px-2 py-1 rounded-full ${p.clasesUsadas >= p.clasesPagadas ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>
                          {p.clasesUsadas >= p.clasesPagadas ? 'Agotado' : 'Activo'}
                        </span>
                      </div>
                      <p className="text-sm text-gray-600">Fecha de pago: {new Date(p.fechaPago).toLocaleDateString("es-ES", { timeZone: "UTC" })}</p>
                      <p className="text-sm text-gray-600 mt-1 mb-3">Usadas: <strong>{p.clasesUsadas}</strong> / Pagadas: <strong>{p.clasesPagadas}</strong></p>
                      <button
                ...
                {Math.ceil(paquetes.length / itemsPorPagina) > 1 && (
                  <div className="flex justify-center items-center mt-6 gap-4">
                    <button
                      onClick={() => setPaginaPaquetes(Math.max(1, paginaPaquetes - 1))}
                    ...
                    <span className="text-sm font-semibold text-gray-600">
                      Página {paginaPaquetes} de {Math.ceil(paquetes.length / itemsPorPagina)}
                    </span>
                    <button
                      onClick={() => setPaginaPaquetes(Math.min(Math.ceil(paquetes.length / itemsPorPagina), paginaPaquetes + 1))}
                      disabled={paginaPaquetes === Math.ceil(paquetes.length / itemsPorPagina)}
                      className="px-4 py-2 bg-indigo-50 text-indigo-700 rounded-xl font-bold hover:bg-indigo-100 disabled:opacity-50 disabled:cursor-not-allowed transition"
                    >
                      Siguiente
                    </button>
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                  ...
                  <table className="min-w-full divide-y divide-gray-200 text-sm">
                    <thead className="bg-gray-50">
                      <tr>
                        ...
                        <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Factura Usada</th>
                        <th className="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase">Acción</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-200">
                      {clases.slice((paginaClases - 1) * itemsPorPagina, paginaClases * itemsPorPagina).map(c => {
                        const infoPaciente = c.ninos.find(n => (n.nino?._id || n.nino) === id);
                        return (
                          <tr key={c._id} className="hover:bg-gray-50">
                            <td className="px-4 py-3 font-medium text-gray-900">{c.nombre}</td>
                            <td className="px-4 py-3 text-gray-500">{c.fecha}</td>
                            <td className="px-4 py-3 text-gray-500">
                              {infoPaciente?.numeroFactura || <span className="text-xs text-red-500 italic">Sin paquete / Clase de prueba</span>}
                            </td>
                            <td className="px-4 py-3 text-center">
                              <button onClick={() => navigate(`/clases/${c._id}`)} className="text-indigo-600 hover:text-indigo-900 font-semibold bg-indigo-50 px-3 py-1 rounded-lg">
                                Ir a Clase
                              </button>
                            </td>
                          </tr>
                ...
                {Math.ceil(clases.length / itemsPorPagina) > 1 && (
                  <div className="flex justify-center items-center mt-6 mb-4 gap-4">
                    <button
                      onClick={() => setPaginaClases(Math.max(1, paginaClases - 1))}
                    ...
                    <span className="text-sm font-semibold text-gray-600">
                      Página {paginaClases} de {Math.ceil(clases.length / itemsPorPagina)}
                    </span>
                    <button
                      onClick={() => setPaginaClases(Math.min(Math.ceil(clases.length / itemsPorPagina), paginaClases + 1))}
                      disabled={paginaClases === Math.ceil(clases.length / itemsPorPagina)}
                      className="px-4 py-2 bg-indigo-50 text-indigo-700 rounded-xl font-bold hover:bg-indigo-100 disabled:opacity-50 disabled:cursor-not-allowed transition"
                    >
                      Siguiente
                    </button>
