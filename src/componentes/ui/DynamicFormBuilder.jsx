...
const CIE10_FISIO_SUGERENCIAS = [
  ...
  {
    codigo: "F900",
    descripcion: "PERTURBACION DE ACTIVIDAD Y ATENCION (TDAH)",
  },
  {
    codigo: "F82",
    descripcion: "TRASTORNO DEL DESARROLLO DE LA FUNCION MOTRIZ",
  },
...
export default function DynamicFormBuilder({
      ...
      esquema.secciones.forEach((seccion) => {
        (seccion.campos || []).forEach((campo) => {
          if (campo.autoNow && campo.tipo === "datetime-local") {
            ...
            defaultData[campo.nombre] =
              campo.valorPorDefecto !== undefined ? campo.valorPorDefecto : "";
          }
          if (campo.tipo === "firma") {
            ...
            if (
              ...
              try {
                ...
                if (
                  data &&
                  data.length > 0 &&
                  data[0].codigo.toUpperCase() === val.toUpperCase()
                ) {
      ...
      (sec.campos || []).forEach((campo) => {
        if (
          ...
          const peso = parseFloat(
            campo.autoCalc.peso === nombre
              ? valor
              : prevData[campo.autoCalc.peso],
          );
          const talla = parseFloat(
            campo.autoCalc.talla === nombre
              ? valor
              : prevData[campo.autoCalc.talla],
          );
          updates[campo.nombre] =
            peso > 0 && talla > 0
              ? (peso / Math.pow(talla / 100, 2)).toFixed(1)
              : "";
        }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      for (const key in finalFormData) {
        ...
        if (
          typeof finalFormData[key] === "string" &&
          finalFormData[key].startsWith("data:image")
        ) {
          console.log(
            `⚠️ Firma Base64 detectada en el campo: ${key}. Iniciando subida a S3...`,
          );
          try {
            ...
            const uploadResponse = await fetch(
              `${API_CONFIG.BASE_URL}/api/upload?origen=firma_paciente&id=${pacienteId || 'nuevo'}`,
              {
                method: "POST",
            ...
            console.log(
              `📥 Respuesta de subida para ${key}: status ${uploadResponse.status}`,
            );
            ...
            console.log(
              `✅ Firma subida a S3 correctamente, URL obtenida:`,
              uploadData.url,
            );
            ...
            Swal.fire(
              "Error",
              "No se pudo subir una de las firmas. Intente de nuevo.",
              "error",
            );
            setGuardando(false);
            return;
      ...
      Swal.fire(
        "¡Éxito!",
        `${esquema.titulo} ${isEdit ? "actualizado" : "guardado"} correctamente.`,
        "success",
      );
      if (onSubmitSuccess) {
        onSubmitSuccess();
  ...
  const renderCampo = (campo) => {
    ...
    if (campo.tipo === "cie10") {
      ...
      const handleCie10Focus = async (e) => {
        ...
        if (
          !currentQuery &&
          (!cie10Results[campo.nombre] ||
            cie10Results[campo.nombre].length === 0)
        ) {
          setCie10Results((p) => ({
            ...p,
            [campo.nombre]: CIE10_FISIO_SUGERENCIAS,
          }));
        }
      };
        ...
        cie10Timers.current[campo.nombre] = setTimeout(async () => {
          ...
          try {
            const data = await apiRequest(
              `/cie10?q=${encodeURIComponent(val)}&limit=15`,
            );
      ...
      return (
        <div className="flex flex-col gap-1 relative">
          ...
          >
            {campo.etiqueta}{" "}
            {campo.requerido && <span className="text-pink-600">*</span>}
          </label>
          <div className="relative">
            <input
              ...
              type="text"
              placeholder={
                campo.placeholder || "Buscar código o descripción..."
              }
              value={query}
            ...
            />
            {isLoading && (
              <span className="absolute right-3 top-3 text-indigo-400 text-xs">
                Buscando...
              </span>
            )}
          </div>
          ...
          {isOpen && results.length > 0 && (
            <ul
              className={`absolute left-0 right-0 z-50 bg-white border border-indigo-200 rounded-xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.2)] max-h-56 overflow-y-auto ${isDropUp ? "bottom-full mb-1 shadow-[0_-10px_40px_-10px_rgba(0,0,0,0.2)]" : "top-full mt-1"}`}
            >
              {results.map((d) => (
                <li
                  key={d.codigo}
    ...
    if (campo.tipo === "cups") {
      ...
      const handleCupsChange = (e) => {
        ...
        cie10Timers.current[campo.nombre] = setTimeout(async () => {
          ...
          try {
            const data = await apiRequest(
              `/cups-catalogo?q=${encodeURIComponent(val)}&limit=15`,
            );
            setCie10Results((p) => ({ ...p, [campo.nombre]: data }));
          } catch {
            /* silenciar */
        ...
        <div className="flex flex-col gap-1 relative">
          ...
          <div className="relative">
            <input
              ...
              type="text"
              placeholder={
                campo.placeholder || "Buscar por CUPS o descripción..."
              }
              value={query}
            ...
            />
            {isLoading && (
              <span className="absolute right-3 top-3 text-blue-400 text-xs">
                Buscando...
              </span>
            )}
          </div>
    ...
    if (campo.tipo === "firma") {
      return (
        <div className="flex flex-col gap-2 md:col-span-2">
          ...
          <button
            type="button"
            onClick={() => handleClearSignature(campo.nombre)}
            className="text-sm text-pink-600 self-end hover:underline"
          >
            Limpiar Firma
          </button>
        </div>
    ...
    if (campo.tipo === "checkbox") {
      return (
        <div className="flex items-start gap-4 md:col-span-2 p-5 bg-indigo-50/20 rounded-2xl border border-indigo-100 transition-all hover:bg-white hover:shadow-md group">
          ...
          <label
            className="text-gray-700 font-bold text-sm cursor-pointer group-hover:text-indigo-900"
            htmlFor={campo.nombre}
          >
            {campo.etiqueta}
          </label>
    ...
    if (campo.tipo === "checkbox_group") {
      ...
      return (
        <div className="flex flex-col gap-2 md:col-span-2 mb-4">
          <label className="text-[11px] font-black text-indigo-400 uppercase tracking-widest mb-1">
            {campo.etiqueta}
          </label>
          <div className="flex flex-row flex-wrap gap-4">
            {campo.opciones.map((opt) => (
              <label
                key={opt.valor}
                className={`flex items-center gap-3 cursor-pointer p-4 rounded-2xl border-2 transition-all shrink-0 ${valoresActuales.includes(opt.valor) ? "bg-indigo-50 border-indigo-500 text-indigo-900 shadow-sm" : "bg-white border-gray-100 text-gray-400 hover:border-indigo-200 shadow-sm"}`}
              >
                <div
                  className={`flex-shrink-0 w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all ${valoresActuales.includes(opt.valor) ? "bg-indigo-600 border-indigo-600" : "bg-white border-gray-200"}`}
                >
                  {valoresActuales.includes(opt.valor) && (
  ...
  const renderSeccion = (seccion, idxSec, isSidebar = false) => {
    ...
    return (
      <div
        key={idxSec}
        className={`rounded-2xl border border-indigo-100 ${isSidebar ? "shadow-sm" : ""}`}
      >
        ...
        <div className="p-4 bg-white rounded-b-xl">
            ...
            <div className="flex flex-col gap-3">
              {camposReq.map((c, i) => (
                <React.Fragment key={`req-${i}`}>
                  {renderCampoConEstilo(c, true, true)}
                </React.Fragment>
              ))}
              {camposOpc.length > 0 && (
              ...
              {camposOpc.map((c, i) => (
                <React.Fragment key={`opc-${i}`}>
                  {renderCampoConEstilo(c, false, true)}
                </React.Fragment>
              ))}
            </div>
          ) : (
              ...
              {camposSeguros
                ...
                .map((c, i) => (
                  <React.Fragment key={i}>
                    {renderCampoConEstilo(c, c.requerido, false)}
                  </React.Fragment>
                ))}
            </div>
          )}
          ...
          {camposTexarea.length > 0 && (
            <div className="space-y-4 mt-4 pt-4 border-t border-indigo-50">
              {camposTexarea.map((c, i) => (
                <React.Fragment key={i}>{renderCampo(c)}</React.Fragment>
              ))}
            </div>
          )}
          {/* Campos ocultos o hidden que no deben ocupar espacio */}
          {camposOcultos.map((c, i) => (
            <React.Fragment key={i}>{renderCampo(c)}</React.Fragment>
          ))}
        </div>
      </div>
  ...
  return (
    <div
      className={
        isModalLayout
          ? "flex-1 w-full bg-white flex flex-col min-h-0"
    ...
    >
      ...
      >
        ...
        <div
          className={isModalLayout ? "flex-1 overflow-y-auto px-6 py-6" : ""}
        >
          {/* Título */}
          {!isModalLayout && (
            <h2 className="text-2xl font-extrabold text-indigo-700 mb-5 text-center">
              {esquema.titulo}
            </h2>
          )}
          ...
          {errores && (
            <div className="mb-4 p-4 bg-red-100 text-red-700 font-semibold rounded-xl">
              {errores}
            </div>
          )}
          ...
          <div
            className={`flex flex-col w-full ${seccionesAlwaysOn.length > 0 ? "lg:flex-row items-start" : "max-w-5xl mx-auto"} gap-6`}
          >
              ...
              <div className="w-full lg:w-[420px] lg:sticky lg:top-4 flex-shrink-0 space-y-4 pr-1 pb-4 z-20">
                <div className="bg-indigo-700 rounded-2xl px-4 py-3 shadow-md flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-indigo-200 text-xs tracking-widest">
                      Siempre Visibles
                    </p>
                  </div>
                </div>
        ...
        >
          {isPaginado ? (
            <>
              ...
              <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto order-1 sm:order-2">
                {pasoActual < totalPasos - 1 ? (
                  ...
                  <button
                    type="submit"
                    disabled={guardando || submitLocked}
            ...
            <div
              className={
                isModalLayout
                  ? "flex flex-col-reverse sm:flex-row gap-3 w-full justify-end"
                  : "flex flex-col-reverse sm:flex-row gap-4 w-full justify-center"
              }
            >
              <button
                type="button"
                ...
                className={
                  isModalLayout
                    ...
                    : "w-full sm:w-auto px-10 py-4 bg-slate-100 text-slate-600 rounded-2xl font-black uppercase tracking-wider hover:bg-slate-200 transition-all text-xs border border-slate-200"
                }
              >
                ✕ Cancelar
              </button>
              <button
                ...
                className={
                  isModalLayout
                    ? "w-full sm:w-auto px-6 py-2.5 font-bold text-white bg-indigo-600 rounded-xl hover:bg-indigo-700 transition-colors shadow-sm disabled:opacity-50"
                    : "w-full sm:w-auto px-10 py-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl font-black uppercase tracking-wider text-xs transition-all shadow-lg shadow-indigo-200 disabled:opacity-50"
                }
              >
