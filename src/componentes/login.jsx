...
export default function Login({ onLogin }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.requiere2FA) {
        ...
        setRequiere2FA(true);
        setTempToken(data.token); // Token temporal
        setUsuarioInfo({ nombre: data.nombre, rol: data.rol });
        setCargando(false);
        return;
      }
  ...
  const handleVerificar2FA = async (e) => {
    ...
    setShake(false);
    try {
      const data = await apiRequest(API_ENDPOINTS.VERIFY_2FA_LOGIN, {
        method: "POST",
        body: JSON.stringify({ token: codigo2FA, tempToken }),
      });
      ...
      {!requiere2FA ? (
        ...
        >
          <div className="text-center">
            <h2 className="text-3xl font-extrabold text-indigo-700 mb-2 drop-shadow">Verificación 2FA</h2>
            <p className="text-gray-600 text-sm">
              Hola <span className="font-semibold text-indigo-600">{usuarioInfo?.nombre}</span>
              <br />
          ...
          <input
            className="border border-indigo-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-indigo-400 bg-indigo-50 shadow-sm text-base text-center text-2xl font-mono tracking-widest"
            value={codigo2FA}
          ...
          <button
            type="submit"
            className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 rounded-xl transition flex items-center justify-center text-lg shadow"
            disabled={cargando || exito || codigo2FA.length !== 6}
          >
            {cargando ? (
              <span className="flex items-center gap-2">
                <svg className="animate-spin h-5 w-5 text-white" viewBox="0 0 24 24">
