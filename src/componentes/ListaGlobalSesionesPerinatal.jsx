...
export default function ListaGlobalSesionesPerinatal() {
    ...
    const cargarDatos = async () => {
        ...
        try {
            ...
            const agrupados = data.reduce((acc, item) => {
                ...
                if (!acc[pacienteId] || new Date(item.createdAt) > new Date(acc[pacienteId].createdAt)) {
                    acc[pacienteId] = item;
                }
                return acc;
            }, {});
    ...
    const pacientesFiltrados = pacientes.filter(p => {
        const nombreCompleto = `${p.paciente?.nombres} ${p.paciente?.apellidos}`.toLowerCase();
        const documento = (p.paciente?.numDocumentoIdentificacion || "").toLowerCase();
    ...
    const calcularProgreso = (sesiones = [], sesionesExtra = []) => {
        ...
        if (total === 0) return 0;
        const completadas = [...sesiones, ...sesionesExtra].filter(s => s.firmaPaciente && s.fecha).length;
                    ...
                    <div className="relative z-10">
                        <div className="flex flex-col md:flex-row justify-between items-center mb-10 gap-6">
                            ...
                            <div className="flex gap-3">
                                ...
                                >
                                    <ArrowPathIcon className={`h-6 w-6 ${cargando ? 'animate-spin' : ''}`} />
                                </button>
                                <button
                        ...
                        {cargando ? (
                            <div className="flex flex-col items-center py-20">
                                ...
                                <span className="mt-4 text-indigo-400 font-black tracking-widest text-xs uppercase animate-pulse">Cargando Programas...</span>
                            </div>
                        ) : (
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {pacientesFiltrados.map((item) => {
                                    const progreso = calcularProgreso(item.sesiones, item.sesionesIntensivo);
                                    return (
                                        <div
                                            key={item._id}
                                        ...
                                        >
                                            <div className="p-6">
                                                <div className="flex items-start justify-between mb-4">
                                                    ...
                                                    <div className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-tighter ${progreso === 100 ? 'bg-green-100 text-green-600' : 'bg-orange-100 text-orange-600'
                                                        }`}>
                                                        {progreso === 100 ? 'Programa Finalizado' : 'En Curso'}
                                                    </div>
                                                </div>
                                                ...
                                                <p className="text-xs font-bold text-gray-400 mb-4 tracking-widest">
                                                    ID: {item.paciente?.numDocumentoIdentificacion || "N/A"}
                                                </p>
                                                ...
                                                <div className="space-y-4 mb-6">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div className="flex-1">
                                                            <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Plan Activo</p>
                                                            <p className="text-xs font-bold text-gray-700 capitalize">
                                                                {item.tipoPrograma || item._datosLegacy?.tipoPrograma || 'No especificado'}
                                                            </p>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div>
                                                        <div className="flex justify-between items-center mb-1">
                                                            <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Progreso del Programa</span>
                                                            <span className="text-xs font-black text-indigo-600">{progreso}%</span>
                                                        </div>
                                ...
                                {pacientesFiltrados.length === 0 && !cargando && (
                                    <div className="col-span-full py-20 text-center bg-white rounded-[3rem] border-2 border-dashed border-indigo-100">
                                        ...
                                        <h3 className="text-xl font-black text-indigo-200 uppercase tracking-tighter">No hay programas perinatales activos</h3>
                                        <p className="text-gray-400 text-sm mt-2">Inicia una nueva valoración para registrar a una paciente.</p>
                                    </div>
                                )}
                            </div>
                        )}
