...
export default function DetalleClase() {
  ...
  useEffect(() => {
    if (ninoId) {
      apiRequest(`/pagoPaquete/por-nino/${ninoId}`).then(setPaquetes);
    } else {
      setPaquetes([]);
      setFacturaAgregar("");
    }
  ...
  useEffect(() => {
    if (asignarPaqueteId) {
      apiRequest(`/pagoPaquete/por-nino/${asignarPaqueteId}`).then(
  ...
  if (!clase)
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-white">
        ...
        <span className="mt-6 text-indigo-700 font-bold text-lg">
          Cargando sesión...
        </span>
      </div>
    );
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-100 via-pink-100 to-green-100 py-10 px-2">
      <div className="max-w-2xl mx-auto p-6 bg-white rounded-3xl shadow-2xl border border-indigo-100">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-3xl font-bold text-indigo-700 drop-shadow">
            {clase.nombre}
          </h2>
          <button
        ...
        {!clase.bloqueada && (
          <div className="bg-indigo-50 border rounded-2xl p-6 mb-8 shadow">
            <h3 className="font-bold text-lg text-indigo-800 mb-4">
              Agregar niño a la clase
            </h3>
            <div className="mb-4">
              <label className="block mb-1 font-medium text-gray-700">
                Buscar y seleccionar niño:
              </label>
                ...
                {n.firma && (
                  <div className="flex flex-col items-center mt-2">
                    <img
                      src={n.firma}
                      alt="firma"
                      className="border rounded max-h-12 bg-white"
                    />
                    {n.auditTrail && (
                      <p className="text-[8px] text-gray-400 font-mono text-center">
                        IP: {n.auditTrail.ip} |{" "}
                        {new Date(n.auditTrail.fechaHora).toLocaleTimeString()}
                      </p>
                    )}
                  </div>
                )}
        ...
        {!clase.bloqueada && (
          <>
            <h3 className="font-semibold mb-2 mt-6 text-indigo-700">
              Firma de niño
            </h3>
            <div className="bg-green-50 border rounded-2xl p-6 mb-6 shadow">
              <label className="block mb-2 font-medium text-gray-700">
                Selecciona un niño para firmar:
              </label>
              <select
                value={firmaNinoId}
              ...
              {clase.ninos.filter((n) => !n.firma).length === 0 && (
                <p className="mt-3 text-sm text-green-700">
                  ✅ Todos los niños ya han firmado esta sesión.
                </p>
              )}
            </div>
          </>
        ...
        {clase.bloqueada && clase.selloIntegridad && (
          <div className="w-full p-4 bg-white border border-green-200 rounded-2xl shadow-sm">
            <p className="text-[10px] font-mono text-green-700 break-all mb-1">
              <strong>SELLO INTEGRIDAD (SHA-256):</strong>{" "}
              {clase.selloIntegridad}
            </p>
            <p className="text-[10px] text-gray-500 italic">
              Libro de asistencia auditado y sellado el{" "}
              {new Date(clase.fechaBloqueo).toLocaleString()}
            </p>
          </div>
        )}
