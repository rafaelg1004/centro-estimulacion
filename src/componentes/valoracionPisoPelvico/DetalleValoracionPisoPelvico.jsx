...
export default function DetalleValoracionPisoPelvico() {
        ...
        if (data.paciente) {
          try {
            ...
            if (typeof data.paciente === 'object' && data.paciente._id) {
              console.log('Paciente ya poblado en valoración:', data.paciente);
              ...
              throw new Error('Formato de paciente no válido');
            }
          } catch (error) {
  ...
  const datosGenerales = [
    ...
    { label: "Cédula", value: paciente?.cedula || valoracion.cedula },
    ...
    { label: "Médico tratante", value: paciente?.medicoTratante || valoracion.medicoTratante },
    { label: "Acompañante", value: paciente?.acompanante },
    { label: "Teléfono acompañante", value: paciente?.telefonoAcompanante },
    { label: "Nombre bebé", value: paciente?.nombreBebe },
    { label: "Semanas gestación", value: paciente?.semanasGestacion },
    { label: "FUM", value: paciente?.fum },
    { label: "Fecha probable parto", value: paciente?.fechaProbableParto },
    { label: "Fecha valoración", value: valoracion.fecha },
  ...
  const enfermedadCronica = [
    ...
    { label: "Observaciones Psicológicas", value: valoracion.observacionesPsico },
          ...
          {paciente && (
            <div className="mt-2 md:mt-0 text-sm text-gray-600 text-center md:text-right">
              ...
              <div>
                ...
                {paciente.cedula || <span className="text-gray-400">Sin cédula</span>}
              </div>
            </div>
          )}
        </div>
            ...
            <div className="flex">
              ...
              <div className="ml-3">
                ...
                <div className="mt-2 text-sm text-yellow-700">
                  <p>No se pudo obtener la información del paciente asociado a esta valoración (ID: {typeof valoracion.paciente === 'object' ? valoracion.paciente?._id || JSON.stringify(valoracion.paciente) : valoracion.paciente}). Es posible que el paciente haya sido eliminado del sistema.</p>
                </div>
              </div>
            </div>
          </div>
